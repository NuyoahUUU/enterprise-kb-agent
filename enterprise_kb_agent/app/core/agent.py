...
CRITICAL — 工具调用规则（必须严格遵守）：
- 你必须通过 API 的 tool_calls 字段来调用工具，系统会自动执行并返回真实结果
- 严禁在回答文本中模拟或伪造工具调用（如伪造命令行输出、伪造文件内容）
- 严禁在没有调用工具的情况下声称"已执行"了某个操作
- 每个工具调用的结果已经以 role="tool" 的形式注入对话，你只需要如实汇报
- 如果你不确定某个工具是否真的执行了，查看上一条 role="tool" 的消息
- 永远不要凭空编造命令执行结果、文件内容、或系统状态信息
"""
...
class ReActAgent:
    ...
    def run(self, question: str, session_id: str | None = None, document_id: str | None = None,
        ...
        while iterations < self.MAX_ITERATIONS:
            ...
            # 检测 LLM 是否在文本中模拟工具调用而非通过 tool_calls 执行
            if self._looks_like_simulated_tools(final_answer) and iterations < self.MAX_ITERATIONS:
                logger.warning("检测到疑似模拟工具输出，强制要求 LLM 使用 tool_calls")
                messages.append({"role": "user", "content": (
                    "你刚才的回答中似乎在文本里模拟了工具调用，而不是通过 tool_calls 字段真正执行。"
                    "系统检测到你没有调用任何工具却声称执行了操作。"
                    "请现在通过 tool_calls 字段重新调用你需要使用的工具，获取真实的执行结果后再回答。"
                    "如果你不需要任何工具就能回答，请直接给出答案，不要伪造工具执行记录。"
                )})
                continue
            break
    ...
    def run_stream(self, question: str, session_id: str | None = None, document_id: str | None = None,
        ...
        while iterations < self.MAX_ITERATIONS:
            ...
            if self._looks_like_simulated_tools(final_answer) and iterations < self.MAX_ITERATIONS:
                logger.warning("检测到疑似模拟工具输出，强制要求 LLM 使用 tool_calls")
                yield self._sse("thought", {"text": "检测到回答中疑似包含未执行的模拟操作，正在要求 Agent 重新通过工具执行...", "phase": "retry"})
                messages.append({"role": "user", "content": (
                    "你刚才的回答中似乎在文本里模拟了工具调用，而不是通过 tool_calls 字段真正执行。"
                    "系统检测到你没有调用任何工具却声称执行了操作。"
                    "请现在通过 tool_calls 字段重新调用你需要使用的工具，获取真实的执行结果后再回答。"
    ...
    # 匹配 LLM 在文本中模拟工具调用的模式
    _SIMULATED_TOOL_PATTERNS = [
        r"(?:🔧|🛠️|🔨)\s*(?:run_command|read_file|write_file|knowledge_search)",
        r"(?:→|→)\s*\{.*(?:returncode|stdout|stderr)",
        r"Agent 已执行 \d+ 个操作",
        r"上一条回复我伪造了",
        r"我伪造了执行结果",
    ]

    def _looks_like_simulated_tools(self, text: str) -> bool:
        """检测 LLM 是否在文本中模拟了工具调用而非通过 tool_calls 执行。"""
        if not text:
            return False
        for pattern in self._SIMULATED_TOOL_PATTERNS:
